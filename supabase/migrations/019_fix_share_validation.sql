-- Fix share percentage validation to work with new UI logic
-- Now: company_rate + sum(academician_shares) = 100%
...
BEGIN
  ...
  IF total_shares != required_academician_total THEN
    RAISE EXCEPTION 'Academician share percentages must equal %%. Company rate: %%, Required academician total: %%, Current academician total: %%',
...
-- The trigger is already created, just the function was updated