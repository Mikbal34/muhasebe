...
export default function EditProjectPage({ params }: { params: { id: string } }) {
  ...
  const validateForm = () => {
    ...
    if (totalPercentage !== 100) {
      newErrors.representatives = `Toplam pay yüzdesi 100 olmalı (şu an: ${totalPercentage})`
    }
  ...
  const addRepresentative = (userId: string) => {
    ...
    if (!userToAdd || representatives.some(rep => rep.user_id === userId)) return
