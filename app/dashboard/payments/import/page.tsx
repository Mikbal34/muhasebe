  ...
  const parseExcelFile = async (selectedFile: File) => {
    ...
    try {
      ...
      const preview: PreviewRow[] = rows.map((row, index) => {
        ...
        if (!projeKodu) errors.push('Proje kodu zorunlu')

        const aliciAdi = row['Alıcı Adı']?.toString().trim() || ''
        if (!aliciAdi) errors.push('Alıcı adı zorunlu')
        ...
        const iban = row['IBAN']?.toString().trim() || ''

        let odemeTarihi = ''
        const rawDate = row['Ödeme Tarihi']
        if (rawDate) {
          if (rawDate instanceof Date) {
            odemeTarihi = rawDate.toLocaleDateString('tr-TR')
          } else if (typeof rawDate === 'number') {
            const date = XLSX.SSF.parse_date_code(rawDate)
            if (date) {
              odemeTarihi = `${String(date.d).padStart(2, '0')}.${String(date.m).padStart(2, '0')}.${date.y}`
            }
          } else {
            odemeTarihi = rawDate.toString().trim()
          }
        }
      ...
      preview.forEach((row) => {
        if (row.hasError && row.errorMessage) {
          row.errorMessage.split(', ').forEach((msg) => {
            allErrors.push({ row: row.rowNumber, column: '', message: msg })
          })
        }
      })
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-navy flex items-center gap-2">
              ...
              Toplu Ödeme Talimatı Aktarımı
            </h1>
            <p className="text-sm text-slate-500">Excel dosyasından birden fazla ödeme talimatı oluşturun</p>
          </div>
        </div>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-emerald-700 mt-1">
                      <span className="font-bold">{importResult.imported}</span> ödeme talimatı başarıyla oluşturuldu.
                      <br />
              ...
              <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                ...
                <div className="p-6">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <a
                      href="/templates/odeme-import-sablonu.xlsx"
                      download
                    ...
                    <table className="w-full text-sm">
                      <thead className="bg-slate-50 text-[11px] font-black text-slate-500 uppercase tracking-widest sticky top-0 z-10">
                        <tr>
                          ...
                          <th className="px-4 py-3 text-left">Alıcı Adı</th>
                        ...
                        {previewData.map((row, idx) => (
                          ...
                          >
                            ...
                            <td className="px-4 py-3">
                              <span className="inline-flex items-center gap-1 px-2 py-1 rounded text-[10px] font-bold bg-navy/10 text-navy">
                            ...
                            <td className="px-4 py-3 font-mono text-xs text-slate-500 max-w-[120px] truncate" title={row.iban}>
                              {row.iban !== '-' ? `...${row.iban.slice(-8)}` : '-'}
                            </td>
                ...
                <div className="space-y-3 text-sm">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500">Örn: YTÜTTO1143</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    <div className="w-6 h-6 rounded bg-red-100 flex items-center justify-center flex-shrink-0">
                    ...
                    <div>
                      <p className="font-semibold text-slate-700">Alıcı Adı</p>
                      <p className="text-xs text-slate-500">Sistemdeki kullanıcı veya personel adı</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    <div className="w-6 h-6 rounded bg-red-100 flex items-center justify-center flex-shrink-0">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500">Ödeme tutarı (TL)</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500">Ödeme açıklaması</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    <div className="w-6 h-6 rounded bg-slate-100 flex items-center justify-center flex-shrink-0">
                    ...
                    <div>
                      <p className="font-semibold text-slate-700">IBAN</p>
                      <p className="text-xs text-slate-500">Doğrulama amaçlı</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    <div className="w-6 h-6 rounded bg-slate-100 flex items-center justify-center flex-shrink-0">
              ...
              <ul className="space-y-3 text-xs text-slate-600">
                <li className="flex items-start gap-3">
                  <div className="w-6 h-6 rounded-lg bg-navy/10 flex items-center justify-center flex-shrink-0">
                    <span className="text-[10px] font-bold text-navy">1</span>
                  </div>
                  <div>
                    <span className="font-bold text-navy">Proje Kodu</span>
                    <p className="text-slate-500 mt-0.5">Sistemdeki proje koduyla birebir eşleşmeli</p>
                  </div>
                </li>
                <li className="flex items-start gap-3">
                  <div className="w-6 h-6 rounded-lg bg-navy/10 flex items-center justify-center flex-shrink-0">
                    <span className="text-[10px] font-bold text-navy">2</span>
                  </div>
                  <div>
                    <span className="font-bold text-navy">Alıcı Adı</span>
                    <p className="text-slate-500 mt-0.5">Kullanıcı veya personel tam adıyla eşleşmeli</p>
                  </div>
                </li>
                <li className="flex items-start gap-3">
                  <div className="w-6 h-6 rounded-lg bg-navy/10 flex items-center justify-center flex-shrink-0">
                  ...
                  <div>
                    <span className="font-bold text-navy">Bakiye Kontrolü</span>
                    <p className="text-slate-500 mt-0.5">Kişinin o projede yeterli bakiyesi olmalı</p>
                  </div>
                </li>
              </ul>
            </div>
